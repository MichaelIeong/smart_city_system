<script type="text/html" data-template-name="DataSource">
    <div class="form-row">
    ...
    <div class="form-row">
        <label for="node-input-datasourceType"><i class="fa fa-database"></i> DataSource Type</label>
...
<script type="text/javascript">
    RED.nodes.registerType('DataSource', {
        ...
        defaults: {
            ...
            sensorId: {value: ""}  // 添加 sensorId 到 node 默认值中
        },
        inputs: 0,
        outputs: 1,
        icon: "bridge.svg",
        label: function () {
            if (this.deviceName) {
                return "DataSource : " + this.deviceName;
            } else if (this.deviceType) {
                return "DataSource : " + this.deviceType;
            } else {
                return "DataSource : ";
            }
        },
        oneditprepare: function () {
            ...
            var sensorIdInput = $("#node-input-sensor-id");  // 获取隐藏字段的引用
            ...
            $.getJSON("http://127.0.0.1:8080/api/node-red/datasourceType", function (list) {
                ...
                list.forEach(item => {
                    datasourceTypeSelect.append(
                        $("<option></option>").attr("value", item.datasourceType).text(item.datasourceType)
                    );
                });
            ...
            $.getJSON("http://127.0.0.1:8080/api/node-red/sensors/" + projectId, function (data) {
                ...
                // 填充location
                var locations = Array.from(new Set(data.map(function (sensor) {
                ...
                locationSelect.off('change').on('change', function () {
                    ...
                    var deviceNames = filteredSensors.map(sensor => ({
                        value: sensor.deviceName,
                        text: sensor.deviceName
                    }));
                    var deviceTypes = filteredSensors.map(sensor => ({
                        value: sensor.deviceType,
                        text: sensor.deviceType
                    }));
                ...
                deviceNameSelect.off('change').on('change', function () {
                    ...
                    // 更新sensorId
                    var selectedSensor = sensorsData.find(sensor => sensor.deviceName === selectedDeviceName);
                    if (selectedSensor) {
                        sensorIdInput.val(selectedSensor.sensorId);  // 设置隐藏的sensorId
                    }
                });
                ...
                deviceTypeSelect.off('change').on('change', function () {
                    ...
                    // 更新sensorId
                    var selectedSensor = sensorsData.find(sensor => sensor.deviceType === selectedDeviceType);
                    if (selectedSensor) {
                        sensorIdInput.val(selectedSensor.sensorId);  // 设置隐藏的sensorId
                    }
                });
        ...
        oneditsave: function () {
            ...
            this.sensorId = $("#node-input-sensor-id").val();  // 保存sensorId
